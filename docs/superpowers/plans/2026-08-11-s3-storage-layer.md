...
// Kept separate from Number() coercion on purpose: Number('') is 0, which is
// finite and non-negative, so `--min-age=` (an unset shell variable, say)
// would otherwise pass validation and silently disable the grace window.
...
const main = async () => {
    ...
    // Quiet:true suppresses success entries but NOT Errors, so per-key
    // failures are only visible if the response is actually read.
